...
const LostItemsPage = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">🔍 All Lost Items</h1>
        ...
        <table className="min-w-full text-sm text-left whitespace-nowrap">
          <thead className="bg-gray-100 text-gray-600 font-semibold">
            <tr>
              ...
              <th className="py-3 px-4 hidden xl:table-cell">Date Lost</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 text-gray-700">
            {loading ? (
              ...
              <tr>
                <td colSpan="6" className="text-center py-8">No lost items found</td>
              </tr>
            ) : (
              items.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-semibold inline-block px-2 py-1 rounded-full bg-yellow-100 text-yellow-700">
                      {item.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell">{item.postedBy?.name || '-'}</td>
